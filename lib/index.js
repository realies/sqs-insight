...
fs.exists(configLocalFile).then(function (exists) {
    ...
    if (config.dynamicEndpoints && typeof config.dynamicEndpoints === 'object' && config.dynamicEndpoints.length > 0) {
        return Q.allSettled(config.dynamicEndpoints.map(SQSTools.listQueues));
    }
    return [];
}).then(function (dynamicEndpointsResult) {
    if (config.endpoints && typeof config.endpoints !== 'object') {
        throw new Error('Invalid endpoints array in config');
    }

    if (dynamicEndpointsResult.length > 0) {
        if (!config.endpoints) {
            config.endpoints = [];
        }
        dynamicEndpointsResult.forEach(function (result) {
    ...
    if (!config.endpoints || config.endpoints.length < 1) {
